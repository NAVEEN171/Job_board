...
interface SingleSliderProps extends React.ComponentProps<typeof SliderPrimitive.Root> {
...
const SingleSlider = React.forwardRef<React.ElementRef<typeof SliderPrimitive.Root>, SingleSliderProps>(
  ({ className, value, onValueChange, min = 0, max = 100, ...props }, ref) => {
    ...
    return (
      <SliderPrimitive.Root
        ...
        onPointerUp={() => setTimeout(()=>{setIsInteracting(false)},300)}  // Detect when dragging ends
        {...props}
      >
        {/* Slider Track */}
        ...
        >
          {/* Conditional Popup */}
           <span
                          className={cn(
                            'absolute transform -translate-x-1/2 -top-8 text-sm font-semibold text-white bg-blue-600 px-1 py-0.5 rounded shadow-md transition-all ease-in-out duration-200',
                            isInteracting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                          )}
                        >
                          {value}
                        </span>
